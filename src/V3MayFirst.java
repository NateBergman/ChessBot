...
public class V3MayFirst {
    ...
    static final int TIME_PER_MOVE = 3000; //how long we take per move in milliseconds
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (x == 1) {
                ...
                int move = iterativeDeepening( true);
                makeMove(move);
                gameMoves.add(move);
            } else if (x == 3) {
                int move = iterativeDeepening(false);
                makeMove(move);
                gameMoves.add(move);
            }
        }
    ...
    public static int startSearch (int depth, boolean whiteMove, long startTime) {
        ...
        if (whiteMove) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (makeMove(m)) {
                    ...
                    transpositionTable.put(hash,new HashEntry(moves,99,true,9999));
                    return m;
                }
                ...
                if (System.currentTimeMillis() - startTime > TIME_PER_MOVE) {
                    if (!transpositionTable.containsKey(hash) || transpositionTable.get(hash).getDepth() < depth) {
                        transpositionTable.put(hash,new HashEntry(moves,depth,false,(int)bestScore));
                    }
                    return moves.get(0);
                }
            }
            if (!transpositionTable.containsKey(hash) || transpositionTable.get(hash).getDepth() < depth) {
                transpositionTable.put(hash,new HashEntry(moves,depth,true,(int)bestScore));
            }
            return moves.get(0);
        } else {
            ArrayList<Integer> moves;
            ...
            if (whiteMove) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    if (score >= beta) {
                        ...
                        moves.add(0,moves.remove(i));
                        if (!transpositionTable.containsKey(hash) || transpositionTable.get(hash).getDepth() < depth) {
                            transpositionTable.put(hash,new HashEntry(moves,depth,false,(int)score));
                        }
                        return score;
                    }
                    if (score > bestScore) {
    ...
    public static int iterativeDeepening(boolean whiteMove) {
        int move = 0;
